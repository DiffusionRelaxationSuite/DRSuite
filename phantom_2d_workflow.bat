...
@REM Note that batch files have no line continuation features, so the
@REM spacing between the commands is here so that the commands line up
@REM when comparing the .bat file to the .sh file
...
@REM "%RootDir%bin\plot_avg_spectra.exe" spect_imfile Phantom2D/Phantom2D_data_ladmm_spect.mat spatmaskfile Phantom2D/Phantom_mask.mat outprefix Phantom2D/Phantom2D_data_ladmm_avg_spectra cbar 0 color jet linewidth 3 ax_scale "log log" ax_lim "[ 0.05 2 10 200]" nlevel 15 file_types "png pdf"
...
@REM "%RootDir%bin\plot_comp_maps.exe" spect_imfile Phantom2D/Phantom2D_data_ladmm_spect.mat outprefix Phantom2D/Phantom2D_data_component_maps spectmaskfile data/Phantom2D_spectrm_mask.mat cbar 1 weights 1.4 1 1 color data/four_color.mat
@REM if %errorlevel% neq 0 exit /b %errorlevel%
